<template>
  <div>
    <GroupTreeNode v-for="g in groupStore.tree" :key="g.id" :group="g" :depth="0" :selected-id="selId" @select="onSelect" />
    <div class="add" @click="startAdd">
      <span v-if="!adding" class="hint">+ 新建分组</span>
...
function onSelect(id: number | null) { selId.value = id; emit('select', id) }
...
<style scoped>
.add { padding: 4px 8px 4px 32px; cursor: pointer; }
