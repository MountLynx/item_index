  ...
  <Teleport to="body">
    <div class="dialog-overlay" @click.self="$emit('close')">
      <div class="dialog">
        ...
        <div class="section">
          <h4>条目类型</h4>
          <div v-for="t in typeStore.types" :key="t.id" class="type-row">
            <span>{{ t.icon }} {{ t.name }}</span>
            <button v-if="t.id > 2" class="danger small" @click="removeType(t.id)">删除</button>
...
.type-row, .field-row { display: flex; align-items: center; justify-content: space-between; padding: 4px 0; font-size: 13px; }
